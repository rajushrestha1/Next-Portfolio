...
const NavBar = () => {
    ...
    return (
      <div className="flex justify-between items-center w-full h-15 bg-slate-700 text-white px-4 fixed top-0 left-0 z-50 border-b border-slate-700/50">
        <div>
          <h1 className=""><img src='logo1.png' alt='logo' className="h-50 w-auto" /></h1>
        </div>
        ...
        <ul className="hidden md:flex gap-1">
          {links.map(({id, link}) => (
            <li key={id} className="relative px-4 py-2 cursor-pointer capitalize text-slate-300 hover:text-cyan-400 transition-all duration-300 rounded-lg hover:bg-slate-700/50">
              <Link 
                href={`/${link}`}  
                className="relative block font-medium text-sm uppercase tracking-wide">
                {link}
              </Link>
            </li>
          ))}
        ...
        {nav && (
          <ul className='flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-linear-to-b from-slate-900 to-slate-800 text-slate-300'>
            {links.map(({id, link})=>(
              <li key={id} className="px-4 cursor-pointer capitalize py-6 text-4xl hover:text-cyan-400 transition-colors">
